...
import { ladungsEinheiten, type BereichInfo } from "@/lib/entsorgung/bereiche";

// ── Entsorgung: Übersicht der Aufträge ───────────────────────────────────────
//
// Dieselbe Liste für Schrottabholung und Batterietransport. Was sich
// unterscheidet, kommt aus `bereich` — siehe src/lib/entsorgung/bereiche.ts.
...
export function EntsorgungUebersicht({ bereich }: { bereich: BereichInfo }) {
  ...
  const liste = api.entsorgung.auftraege.useQuery({ bereich: bereich.key });
  const anlegen = api.entsorgung.auftragAnlegen.useMutation({
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-xl font-black text-[#1a1a1a] dark:text-[#e4e6eb]">
            {bereich.icon} {bereich.titel}
          </h1>
          <p className="text-sm text-[#65676b] dark:text-[#b0b3b8]">
            Ein Auftrag ist eine Abholung. {bereich.einheitMehrzahl} werden im Auftrag
            erfasst, am Ende geht die Tabelle als E-Mail an den Entsorger.
          </p>
        </div>
        {darfBearbeiten && (
          <button
            onClick={() => { setNeuOffen((v) => !v); setBezeichnung(`${bereich.titel} ${heuteText()}`); }}
        ...
        <div className="rounded-xl border border-[#008BD2]/40 bg-[#008BD2]/5 p-5 space-y-3">
          <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
            <div>
              ...
              <input value={bezeichnung} onChange={(e) => setBezeichnung(e.target.value)} className={eingabe} />
            </div>
            <div>
      ...
      {liste.data && liste.data.length > 0 && (
        <div className="rounded-xl border border-[#ced4da] dark:border-[#3e4042] bg-white dark:bg-[#242526] overflow-hidden divide-y divide-[#ced4da] dark:divide-[#3e4042]">
          {liste.data.map((a) => {
            ...
            const ladung = ladungsEinheiten(bereich, a.anzahl);
            return (
              <Link key={a.id} href={`${bereich.pfad}/${a.id}`}
                ...
                <div className="text-sm text-[#65676b] dark:text-[#b0b3b8] tabular-nums">
                  <b className="text-[#1a1a1a] dark:text-[#e4e6eb]">{a.anzahl}</b> {bereich.einheitMehrzahl}
                  {bereich.ladungGerechnet && <> · {ladung} {bereich.ladungName}</>} ·{" "}
