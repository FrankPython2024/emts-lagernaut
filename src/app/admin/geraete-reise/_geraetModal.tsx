...
function GeraetModal({ initialQ, onClose }: { initialQ: string; onClose: () => void }) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Geräte-Reise"
      className="fixed inset-0 z-[60] flex items-start sm:items-center justify-center bg-black/60 p-0 sm:p-4 overflow-y-auto"
      onClick={onClose}
    >
      <div
        className="bg-[#f0f2f5] dark:bg-[#18191a] w-full sm:max-w-2xl sm:rounded-2xl shadow-2xl my-0 sm:my-6 min-h-screen sm:min-h-0"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Kopf mit Schließen */}
        <div className="sticky top-0 z-10 flex items-center justify-between gap-3 px-5 py-3 bg-[#202F61] text-white sm:rounded-t-2xl">
          <h2 className="font-black flex items-center gap-2"><span aria-hidden>🧭</span> Geräte-Reise</h2>
          <button
            onClick={onClose}
            aria-label="Schließen"
            className="inline-flex items-center justify-center rounded-lg bg-white/15 hover:bg-white/25 font-bold text-lg min-h-[44px] min-w-[44px]"
          >
            ✕
          </button>
        </div>
